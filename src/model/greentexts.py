...
def init():
    # Optional: set start method only once in __main__
    pass


def make_tokenizer():
    # Create a new encoder per process
    return tiktoken.get_encoding("gpt2"), tiktoken.get_encoding("gpt2")._special_tokens[
        "<|endoftext|>"
    ]


def tokenize(doc):
    enc, eot = make_tokenizer()
    tokens = [eot]
    if "text" in doc:
        text = doc["text"]
...
if __name__ == "__main__":
    mp.set_start_method("spawn", force=True)  # key change
        ...
        for tokens in pool.imap(tokenize, ds, chunksize=16):
            if token_count + len(tokens) < shard_size:
                ...
                filename = os.path.join(
                    DATA_CACHE_DIR, f"greentext_{split}_{shard_index:06d}"
                )
